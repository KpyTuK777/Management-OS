...
| Operational Memory (Canonical) | Provide a zone for inspecting the separate Memory model at its current maturity | Query/projection over Memory Event history | Retrieval, explanation, rejected paths, and historical continuity without owning present truth | Inspectable throughout the Matter life; emphasized when history is requested | The Memory model is never absent after Matter opening; the inspection Zone may be closed |
